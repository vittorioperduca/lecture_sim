...
* ...the distribution of $T$ under H0 is not known (analytical derivation of its densities and CDF not possible, large sample approximations not available or not applicable) but it is possible to sample it<br> 
...
$\Rightarrow$ we can calculate the <font color='turquoise'>empirical power:</font>
    ...
    $$
    where $T_\alpha$ is the $(1-\alpha)$-quantile of $T$ under H0.
    If the distribution of T under H0 is not known, one can sample T under H0 and take the $(1-\alpha)$-quantile.
...
Afterwich, the p value and power are estimated as above.
...
# Simulations for case/control testing: emprirical p values
...
We calculate p values $p_{obs,i}$ for individual SNPs and compute $T_{obs}=\min_{i=1,\ldots,10} p_{obs,i}$
```{r}
pvalues_obs=c(NA,10)
...
##
We calculate p values $p_i$ for each permutation $r$ and compute $T^r=\min_{i=1,\ldots,10} p^r_i$
```{r,eval=FALSE}
...
- In these cases power is usually estimated by analaysing many data replicates under H1
...
* Two alternative approaches:
    ...
    1. Simulation of both genotypes and phenotypes for a very large population:
        + Simulate $m$ genotpic datasets, with $m>>n$
...
* idea is to sample recursively phenotypes. Let $Z_i$ the number of cases affected for individuals $1,\ldots,i$, then the steps are 
    ...
    + drawn $Y_3$ conditionally on $Z_2$ and $Z_n=n1$
    + etc
...
* We calculate the empirical power of the test looking for a signal in the 10 SNPs by simulating phenotypes under H0 using waffect
* Fr H1 we take an additive disease model with prevalence $f_0=0.1$ and OR $=1.6$ on SNP 8: 
$$
\pi_i= \frac{1}{1+\exp(-\log(f0)-\log(OR)\times snp_8)}
$$
* Recall that 
    + $T=\min_{i=1,\ldots,10} p_i$
    + $\hat{\mbox{power}} = \frac{\#\{ \mbox{ H1 } T \mbox{ realizations }  > T_\alpha\}}{N}$.  
...
* If all tests are independent (not realistic beacause of LD):
$$
FWER = Pr(\exists ~~1 \leq j \leq p \mbox{ t.q. } H0_{j} \mbox{ rejected } | \forall~~ j~ H0_{j} \mbox{ holds})  = 1-(1-\alpha)^p
$$
...
* $\frac{\alpha'}{p}$ is very small $\Rightarrow$ lose in power

* Lose in power is even more important if tests are dependent: if there are only $q$ truly independent tests out of $p$, ideally one would need to take 
$$
\frac{\alpha'}{q} > \frac{\alpha'}{q}
$$
...
* We show that in this case, the FWER estimate if we test each SNP at the threshold 
$$
...
## Example: dependent SNP

We repeat the very same analysis using a dataset of 10 correlated SNPs and show that the FWER estimate is lower than $\alpha'=0.05$
