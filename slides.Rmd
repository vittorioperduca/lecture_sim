...
* The indicator variables $\mathbb{1}_{T_1>T_{obs}},\ldots,\mathbb{1}_{T_N>T_{obs}}$ are an i.i.d. sample from a Bernoulli whose expected value is the p value. 

* Hence:
$$
\frac{1}{N}\sum_{i=1}^N \mathbb{1}_{T_i>T_{obs}} = \frac{\#\{ T \mbox{ realizations }  > T_{obs}\}}{N} \xrightarrow{P} p
$$
and $\hat{p}$ is a consistent estimator of $p$
...
In this case, drawing a sample of the statistic under a given hypothesis requires two steps:

- simulate <font color='magenta'>$N$ data raplicates</font> according to the hypothesis
- for each replicate $i$, calculate the statistic realization $T_i$ 

Afterwich, the p value and power are estimated as above.


# Simulations for case/control testing 


## Focus on permutation-like methods

In the following we focus on two ways of generating data replicates for testing whether two groups (eg cases vs controls) are the same:

1. <font color='turquoise'>Permutations</font> for simulating under H0 (classic)
2.  <font color='turquoise'>Weighted permutations</font> for simulating under H1 (more original)
...
pvalues=matrix(NA,nrow=10,ncol=1000) #stores the SNP p values in columns, one per replicate
for(repl in 1:1000){ perm=sample(pheno)
  for(snp in 1:10){
    pvalues[snp,repl]=fisher.test(perm,geno[,snp])$p
  }
}
T_sample=apply(pvalues,2,min)  
```
...
## Simulations outside testing: the bootstrap
To do 
